...
### `page.tsx` — Home (`/`)
- `"use client"`. 앱 메인 화면. 상단 헤더 + 좌측 메인 영역 + 우측 대화 패널의 3분할 레이아웃
- 세 가지 `mode` 상태 관리 (`idle` / `recording` / `review`)
  - `idle` — 환자 미선택 시 `EmptyState`, 선택 시 `PatientSessionsPanel` (과거 진료 리스트)
  - `recording` — `RecordingIndicator` (녹음 중/일시정지/오류 표시)
...
- 헤더 구성(`grid-cols-3`): 좌측 `SettingsButton` + 제목, 중앙 `RecordToggle`, 우측 `PatientSelector`
